                ...
                <v-card-text class="px-0">
                    <div class="responsive-datatable">
                        <v-data-table-server
                            ...
                            <template #item.check_out="{ index, item }">
                                <td :data-label="t('booking.check_out')">
                                    <div>
                                        {{ item.check_out }}
                                        <v-btn size="30" :color="new Date(item.check_out).toLocaleDateString() === new Date().toLocaleDateString()?'red':'primary'" variant="flat"
                                            v-if="item.room?.booked" @click="dischargeRoom(index, item.room?.id)">
                ...
                <v-form ref="form">
                    <v-row>
                        ...
                        <v-col cols="12" sm="6" class="pb-0">
                            ...
                            <v-select v-model="item.room" :items="emptyRooms" item-title="name" return-object :rules="namerules" transition="fade-transition" class="mt-1" hide-details density="compact" flat variant="outlined"></v-select>
                        </v-col>
                        <v-col cols="12" class="pb-0">
                            <v-sheet width="100%" rounded class="px-3" border>
                                <v-row>
                                    <v-col cols="2" v-for="room in rooms" :key="room.id" class="d-flex flex-column align-center pt-5 pb-4">
                                        <v-chip label variant="flat" :color="room.booked?'red':'green'">
                                            {{ room.name }}
                                        </v-chip>
...
import { get_rooms_by_type, discharge_room } from '../api/room'
import { get_types } from '../api/roomtype'
// import vMask from '@titou10/v-mask'
import { IBooking, IRoom, IRoomType } from '../interfaces'
...
const getRooms = async (item: IRoomType) => {
    if(!item?.id) return 
...
const loadItems = async () => {
    loading.value = true
    const { data } = await bookingApi.get_staff_bookings(qs.value)
...
const dischargeRoom = async (i: number, id: string) => {
    if(!id) return

    const { data } = await discharge_room(id)
    if(data.status === "ok") {
        Object.assign(items.value[i].room, {
            ...items.value[i].room,
            booked: false
        })
    }
